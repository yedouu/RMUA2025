...
ros::Publisher pub_odom_changedYaw;  // 发布器，用于发布 反转过yaw角的Odometry 消息
...
void callback(const geometry_msgs::PoseStamped::ConstPtr& msg, tf2_ros::Buffer& tfBuffer)
{
    try
    {
        ...
        odom_msg_2.header = point_out_flu.header;
        odom_msg_2.header.stamp = msg->header.stamp;
        odom_msg_2.header.frame_id = "origin";  // 设置参考坐标系

        // 里程计位置（从转换后的坐标系位置）
        odom_msg_2.pose.pose.position.x = point_out_flu.pose.position.x;
        odom_msg_2.pose.pose.position.y = point_out_flu.pose.position.y;
        odom_msg_2.pose.pose.position.z = point_out_flu.pose.position.z;

        // 反转yaw角
        tf2::Quaternion qtn;
        qtn.setRPY(roll, pitch, -yaw);
        odom_msg_2.pose.pose.orientation.x = qtn.getX();
        odom_msg_2.pose.pose.orientation.y = qtn.getY();
        odom_msg_2.pose.pose.orientation.z = qtn.getZ();
        odom_msg_2.pose.pose.orientation.w = qtn.getW();



        // 发布 Odometry 消息
        pub_odom.publish(odom_msg);
        pub_odom_changedYaw.publish(odom_msg_2);    //反转yaw角的里程计
...
int main(int argc, char** argv)
{
    ...
    pub_odom_changedYaw = nh.advertise<nav_msgs::Odometry>("/odom_flu_nav_changedYaw", 10);
