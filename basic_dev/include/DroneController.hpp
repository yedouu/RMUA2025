...
public:
    ...
    void XY_PID_out(float targetX, float targetY, float &X_pid_out, float &Y_pid_out){
        ...
        float xError = (targetX - current_XY[0])*cos(currentYaw) - (targetY - current_XY[1])*sin(currentYaw);
        

        //计算y位置误差
        // float yError = targetY - current_XY[1];
        float yError = (targetX - current_XY[0])*sin(currentYaw) + (targetY - current_XY[1])*cos(currentYaw);
        ...
        //打印PID输出
        ROS_INFO("X_PID_out:%f,Y_PID_out:%f,xError:%f,yError:%f",X_pid_out,Y_pid_out, xError, yError);
    ...
    void XY_vel_PID_out(float targetX_vel, float targetY_vel, float &X_pid_out, float &Y_pid_out){
        ...
        //计算x速度误差
        float x_velocityError = targetX_vel - (currentVelocity_x*cos(currentYaw) - currentVelocity_y*sin(currentYaw));
        // float x_velocityError = (targetX_vel - currentVelocity_x)*cos(-1*currentYaw) + (targetY_vel - currentVelocity_y)*sin(-1*currentYaw);
        //计算y速度误差
        float y_velocityError = targetY_vel - (currentVelocity_x*sin(currentYaw) + currentVelocity_y*cos(currentYaw));
    ...
    void position_control(float targetX,float targetY,float targetZ,float targetYaw,float &pwm0,float &pwm1,float &pwm2,float &pwm3){
        ...
        //打印当前位置
        ROS_INFO("current_X:%f,current_Y:%f,current_Z:%f,current_Yaw:%f",current_XY[0],current_XY[1],currentHeight,currentYaw);
        //高度控制(计算油门)
        ...
        if(yawError <= 6.28/24){
            ...
            //期望速度控制
            XY_vel_PID_out(0, 0, X_vel_pid_out, Y_vel_pid_out);
        }
        ...
        pwm0 = thrust_out - roll_vel_out*0.004 + pitch_vel_out*0.004 + Yaw_vel_pid_out*0.002;
        pwm1 = thrust_out + roll_vel_out*0.004 - pitch_vel_out*0.004 + Yaw_vel_pid_out*0.002;
        pwm2 = thrust_out + roll_vel_out*0.004 + pitch_vel_out*0.004 - Yaw_vel_pid_out*0.002;
        pwm3 = thrust_out - roll_vel_out*0.004 - pitch_vel_out*0.004 - Yaw_vel_pid_out*0.002;



    }

    void velocity_control(float targetX_vel,float targetY_vel,float targetZ_vel,float targetYaw,float &pwm0,float &pwm1,float &pwm2,float &pwm3){
        if(!is_imu_receive || !is_odom_receive){
            ROS_INFO("wait for imu and odom data");
            return;
        }
        
        float vx = targetX_vel;
        float vy = targetY_vel;

        targetX_vel = vx*cos(currentYaw) - vy*sin(currentYaw);
        targetY_vel = vx*sin(currentYaw) + vy*cos(currentYaw);

        //打印目标速度
        ROS_INFO("targetX_vel:%f,targetY_vel:%f,targetZ_vel:%f,targetYaw:%f",targetX_vel,targetY_vel,targetZ_vel,targetYaw);
        
        //高度速度控制(计算油门)
        float thrust_out = height_velocityPID_out(targetZ_vel);
        ...
        if(yawError <= 6.28/12){
            //期望速度控制
            XY_vel_PID_out(targetX_vel, targetY_vel, X_vel_pid_out, Y_vel_pid_out);
        }
        else{
            //控制水平速度为0
